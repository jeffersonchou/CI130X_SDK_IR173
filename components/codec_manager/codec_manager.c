...
static int config_iis_dma(codec_info_t * p_codec_info, io_direction_t io_dir)
{
    ...
    {
        clk = get_ipcore_clk()*get_freq_factor();
    }
...
{    
    ...
    {        
        ...
        {
            xQueueSendFromISR(play_buffer_queue->idle_queue, &(play_buffer_queue->data_begin_addr), xHigherPriorityTaskWoken);
            ci_logdebug(LOG_AUDIO_PLAY, "idle,%x\n", play_buffer_queue->data_begin_addr);
            extern void send_one_buf_done_event(void * xHigherPriorityTaskWoken);   //TODO HSL
            if (uxQueueMessagesWaitingFromISR(play_buffer_queue->idle_queue) > 1)
            {
                //TODO 启动了播报才能打开这个
                //注意，非常注意
