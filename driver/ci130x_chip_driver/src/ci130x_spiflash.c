...
static int32_t spic_write_status_register(spic_base_t spic,char reg1,char reg2,char reg3)
{
    ...
    if(spic_write_by_cpu(spic,&spic_base_config,&temp[2],2))
    {
        return RETURN_ERR;
    }
    return RETURN_OK;
...
int32_t spic_quad_mode(spic_base_t spic)
{
    ...
    {
        reg2 |= (0x1 << 1);
        if(RETURN_ERR == spic_write_status_register(spic,reg1,reg2,spi_flash_drv0_drv1))
        {
            return RETURN_ERR;
        }
    }
