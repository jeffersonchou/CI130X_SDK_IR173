...
#define CWSL_REG_TIMES              (2)             // 学习时 每个词需说几遍，默认 1 遍即可,支持1、2遍,最大支持 2 遍;
...
#define FOR_REG_2TIMES_FLOW_V2      (0)             // 学习时，说两遍逻辑，版本二流程，后续均和第一次的比较，一致学习成功，不一致，最多支持说 3 次
    ...
    if (ciss_get(CI_SS_START_SLEEP_PROCESS) == 0)
    {
        ...
        if (cwsl_app.app_mode == CWSL_APP_REG)
        {
            ...
            else if (CWSL_REG_FINISHED == result)
            {
                ...
                if (CMD_WORD == cwsl_app.word_type && CWSL_CMD_NUMBER > cwsl_tm_get_reg_tpl_number(CMD_WORD) && (cwsl_app.word_id < (CWSL_CMD_NUMBER - 1)))
                {
                    //为了配合学习下一个的逻辑，加了判断条件(cwsl_app.word_id < (CWSL_CMD_NUMBER - 1))
